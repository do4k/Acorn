...
public class PartyRequestClientPacketHandler(ILogger<PartyRequestClientPacketHandler> logger, IWorldQueries worldQueries)
    : IPacketHandler<PartyRequestClientPacket>
{
    public async Task HandleAsync(PlayerState player, PartyRequestClientPacket packet)
    {
        ...
        {
            logger.LogWarning("Player {SessionId} attempted to party action without character or map", player.SessionId);
            return;
        }
