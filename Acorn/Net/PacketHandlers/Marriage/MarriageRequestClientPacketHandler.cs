...
public class MarriageRequestClientPacketHandler(ILogger<MarriageRequestClientPacketHandler> logger, IWorldQueries worldQueries)
    : IPacketHandler<MarriageRequestClientPacket>
{
    public async Task HandleAsync(PlayerState player, MarriageRequestClientPacket packet)
    {
        ...
        {
            logger.LogWarning("Player {SessionId} attempted to marriage action without character or map", player.SessionId);
            return;
        }
